...
export default function Header() {
  ...
  return (
    <nav className="fixed top-0 w-full bg-white/80 dark:bg-gray-900/80 backdrop-blur-md z-50 transition-all duration-300 h-20">
      <div
        className={`h-full py-0 px-10 md:px-20 lg:px-40 flex justify-between items-center ${
          isScrolled
      ...
      >
        ...
        >
          ...
          <span className="text-sm hidden md:inline">{darkMode ? 'Light Mode' : 'Dark Mode'}</span>
        </motion.button>
      </div>
    </nav>
  );
